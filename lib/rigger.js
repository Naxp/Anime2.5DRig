...
(function (root, factory) {
  ...
  // ---------- layer naming ----------
  function normName(n) {
    n = (n || '').normalize('NFKC').trim().replace(/ のコピー\s*\d*$/,'').toLowerCase();   // 'のコピー' = Photoshop duplicate suffix
    ...
    if (n === 'レイヤー 1') n = 'facedetail';   // 'レイヤー 1' = "Layer 1" in Japanese Photoshop
    return n;
  }
  ...
  // recurse into PSD group/folder children, collecting image layers
  function collectImageLayers(node, out) {
    (node.children || []).forEach(function(c){
      // skip hidden subtrees (groups where visible===false)
  ...
  function buildRig(psd, opts) {
    ...
    var W = psd.width, H = psd.height;
    var warnings = [];
    var kids = collectImageLayers(psd, []);
    if (!kids.length) throw new Error('No image layers found (check that the PSD has visible layers)');
    ...
    var byName = {};
    entries.forEach(function (e) { if (byName[e.name]) warnings.push('Duplicate layer name "' + e.name + '" — only the last one will be used');
      byName[e.name] = e; });
    ...
    if (faceE) {
      ...
      warnings.push('No face layer found — using canvas center as face position');
    ...
    for (var ei = 0; ei < entries.length; ei++) {
      ...
      if (!slot) {
        ...
        warnings.push('Unknown layer name "' + e.name + '" — treating as ' + slot.group);
      }
      if (slot.split) {
        ...
        if (!got) warnings.push('"' + e.name + '" left/right separation failed (empty layer?)');
    ...
    if (!anchors.eyeL || !anchors.eyeR) warnings.push('Eye anchors incomplete (check eyewhite/irides layers)');
    ...
    if (mSrc) {
      ...
      warnings.push('No mouth_open / mouth_close found');
    ...
    if (G) {
      ...
      if (G.eyeL && G.eyeR && !findPart('eye_close').length && anchors.eyeL && anchors.eyeR) {
        ...
        synth.eye = true;
        warnings.push('No eye_close layer found — auto-placed generic closed eyes (adjust with eye diff sliders)');
      }
      // mouth
      if (G.mouth && !findPart('mouth_close').length && byName['mouth_open']) {
        ...
        synth.mouth = true;
        warnings.push('No mouth_close layer found — auto-placed generic closed mouth (adjust with mouth sliders)');
      }
    }
